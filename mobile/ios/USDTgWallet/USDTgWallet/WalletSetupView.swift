...
struct WalletSetupView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .sheet(isPresented: $showingMnemonic) {
                MnemonicDisplayView(mnemonic: generatedMnemonic)
            }
            .sheet(isPresented: $showingSuccess) {
                WalletSuccessView(walletName: walletName)
            }
        }
    }
            ...
            Button(action: { selectedOption = 0 }) {
                HStack(spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text("Generate a new wallet with seed phrase")
                            .font(.caption)
                            .opacity(0.8)
                    }
    ...
    private var inputFieldsView: some View {
        VStack(spacing: 16) {
            ...
            // Import Fields (only show if importing)
            if selectedOption == 1 {
                VStack(spacing: 12) {
                    // Seed Phrase Import
                    VStack(alignment: .leading, spacing: 8) {
    ...
    private var actionButtonView: some View {
        Button(action: handleWalletAction) {
            HStack {
                ...
                Text(selectedOption == 0 ? "Create Wallet" : "Import Wallet")
                    .font(.headline)
                    .fontWeight(.semibold)
            }
        ...
        NavigationView {
            VStack(spacing: 24) {
                ...
                VStack(spacing: 16) {
                    ...
                    Text("Write down these 12 words in order. This is the ONLY way to recover your wallet if you lose your device.")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                        .multilineTextAlignment(.center)
                }
                ...
                VStack(spacing: 16) {
                    HStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text("You'll receive 10 USDTg as a welcome gift")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
